...
**Status**: PARTIAL. `WidgetThemeTest`, `WidgetStringsTest`, widget data/provider tests, and worker logic tests cover freshness badges, icon mapping, widget string/accessibility formatting, cache models, saved-city rows, and refresh planning. Full Glance widget unit tests require Glance 1.2.0+ (repo is on 1.1.1). Keep adding pure-function tests until Glance upgrade lands.
