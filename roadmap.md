...
**Status**: ongoing. Baseline is down to **14 IDs** after extracting `RadarPreviewCard`, `CurrentConditionsHeader`, `CustomAlertsScreen.RuleEditor`, `SunArc`, `TemperatureGraph`, `WidgetRefreshWorker`, `SyncedWeatherStore`, and `CompareScreen` helpers/payloads on 2026-05-17, then regenerating `config/detekt/baseline.xml`. Earlier delta: **-2 entries** (MeteoconMapper.getLottieAsset and OwmConditionMapper.toWmoCode refactored from 28-/40-branch `when` expressions to O(1) static lookup maps); **+3 entries** captured during the i18n sweep are now cleared. Remaining baseline entries are broader structural targets: `MainScreen`, `SettingsScreen`, `RadarScreen`, and the `MainViewModel` constructor seam.
