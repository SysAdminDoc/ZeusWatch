...
**Version**: v1.14.0 (phone versionCode 75, wear versionCode 53)
**Package**: `com.sysadmindoc.nimbus`
**Stack**: Kotlin 2.1.0, Jetpack Compose (BOM 2024.12), Hilt 2.53.1, Retrofit 2.11.0, Room 2.6.1, DataStore, MapLibre 11.5.2, Glance 1.1.1, WorkManager 2.10.0, Lottie 6.6.2, Coil 3.0.4, Firebase Firestore, Play Services Wearable 18.2.0
**License**: LGPL-3.0
...
### Wear OS DataLayer Sync
- Phone pushes simplified weather data to watch via `DataClient.putDataItem()` after every fetch
- `WearSyncManager` in `standard` source set (no-op in `freenet`) — fires from `MainViewModel.fetchWeather()` parallel sub-fetch block
- Watch receives via `WeatherDataListenerService` (WearableListenerService) → persists in `SyncedWeatherStore` (SharedPreferences)
- `WearWeatherRepository` prefers synced data (<30 min fresh), falls back to direct Open-Meteo API call when phone disconnected
- Tile and complication services also check synced store first

### Build Variants
- `standard`: Google Play Services (FusedLocationProvider, Wearable DataLayer) + Gemini Nano AI
- `freenet`: F-Droid compatible, no proprietary deps (WearSyncManager is no-op)
...
- **v1.14.0** — **Wear OS Alerts, Daily Forecast, AQI**: Extended DataLayer sync to push weather alerts, 7-day daily forecast, and air quality data to the watch alongside existing current+hourly data. Three new watch screens: **AlertsScreen** with severity-colored dot indicators, event names, headlines, and expiry times; **DailyScreen** with 7-day ScalingLazyColumn showing day label, weather emoji, high/low temps, and precip chance; AQI detail chip on CurrentScreen replacing precip when AQI data available (color-coded emoji by EPA level). **Alert banner** on CurrentScreen — orange background strip showing top alert event name (or count if multiple), tappable to navigate to AlertsScreen. Navigation expanded: CurrentScreen now shows "Hourly" and "7-Day" links side by side, plus alert banner tap. WearNavHost routes extended with `DAILY` and `ALERTS`. **Background sync fix**: `WidgetRefreshWorker` now calls `wearSyncManager.syncWeather()` after primary weather fetch, keeping watch data fresh during background-only refresh cycles when phone app isn't open. Sync timing restructured: phone now syncs AFTER parallel sub-fetches (alerts, AQI) complete so watch receives all data types in a single push. New data models: `WearDailyEntry`, `WearAlertEntry` added to wear module. `WearWeatherData` and `WearUiState` extended with daily, alerts, aqi, aqiLabel fields. `SyncedWeatherStore` and `WeatherDataListenerService` handle all new fields. Wear versionCode 52→53.
- **v1.13.0** — **Wear OS DataLayer Sync**: Phone-to-watch weather data synchronization via Google Play Services Wearable DataLayer API. Phone pushes simplified weather data (`DataMap` with current conditions + 12-hour hourly forecast) to paired watch after every successful weather fetch — eliminates redundant API calls from the watch, reducing battery drain and ensuring phone/watch data consistency. `WearSyncManager` created with flavor-split pattern: real `DataClient.putDataItem()` implementation in `standard` source set, no-op in `freenet`. Watch-side `WeatherDataListenerService` (WearableListenerService) receives data events and persists to `SyncedWeatherStore` (SharedPreferences). `WearWeatherRepository` now checks synced store first — if data is fresh (<30 min), returns immediately without network call; falls back to direct Open-Meteo API when phone is disconnected or data is stale. Tile (`WeatherTileService`) and complication (`WeatherComplicationService`) services also prefer synced data. DataMap path: `/weather/current` with scalar fields + `DataMapArrayList` for hourly entries. ProGuard rules added for Wearable API in both phone and wear modules. Wear versionCode 51→52, versionName 1.10.0→1.13.0.
